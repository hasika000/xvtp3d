...
def main():
    ...
    parser.add_argument('--fpv_type', default='ego')
...
def data_split(data_dir):
    for type1, type2 in [('train', 'full'), ('train', 'subset'),
        ...
        if type1 == 'test':
            ...
            if type2 == 'full':
                with open(os.path.join(data_dir, type1 + '_full_idx.pkl'), 'rb') as f:
                    index_list = pickle.load(f)
            else:
                with open(os.path.join(data_dir, type1 + '_idx.pkl'), 'rb') as f:
                    index_list = pickle.load(f)
